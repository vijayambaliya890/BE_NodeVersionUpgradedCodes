...
class cron {
  ...
  async userIntegrationFirst(req, res) {
    try {
      let companyId = '5a9d162b36ab4f444b4271c8';
      // Get Company Data
      ...
      let timeStamp = `${getYear}${month}${day}${'04'}`;
      let daily;
      let newFile;
      ...
      await sftp
        ...
        .then(async (data) => {
          for (var i = 0; i < data.length; i++) {
            if (data[i].name.indexOf(`dailyMySATS${timeStamp}`) > -1) {
              daily = data[i].name;
              console.log('daily', daily);
              sftp
                .get(`/ADDailyExtractPRD/${data[i].name}`)
                .then((fileData) => {
                  __.log(fileData);
                  let writtenFileData = fs.writeFileSync(
                    `public/${daily}`,
                    fileData,
                  );
                });

              newFile = await spawn('unzip', [
                '-P',
                'P@55word!',
                '-d',
                './public/',
      ...
      //   var newFile = await spawn('unzip', ['-P', 'P@55word!', '-d', './public/', './public/dailyMySATS20190412140104.zip'])
      let serverFile = `./public/${resDaily}${'.csv'}`;
      var columns = [
        'SNo',
        'EmployeeNumber',
        'SamAccountName',
          ...
          if (excelData.length > 0) {
            ...
            for (let elem of userData) {
              ...
              if (elem.Title == null || elem.Title == '') {
                ...
                appointment = `${elem.Title}`;
              }

              let bu;
              if (elem.Department != null && elem.Department != '') {
                bu = `${companyName}>${elem.Company}>${
                  elem.Department
                }>${'__'}`;
              } else {
                bu = `${companyName}>${elem.Company}>${'__'}>${'__'}`;
              }
              ...
              if (elem.Company == null || elem.Company == '') {
                userMissedData = {
                ...
                userMissedData = {
                  ...
                  MobilePhone: elem.MobilePhone,
                  EmailAddress: elem.EmailAddress,
                  Reason: 'Employee Number MisMatch',
                };
                nonUpdated.push(userMissedData);
                continue;
              } else if (elem.DisplayName == null || elem.DisplayName == '') {
                userMissedData = {
                  EmployeeNumber: elem.EmployeeNumber,
                  DisplayName: elem.DisplayName,
                  MobilePhone: elem.MobilePhone,
                  EmailAddress: elem.EmailAddress,
                  Reason: 'Staff Name MisMatch',
                };
                nonUpdated.push(userMissedData);
                continue;
              }
              ...
              if (user.parentBussinessUnitId === null) {
                ...
                // find department..
                let departmentIds;
                departmentIds = await Department.findOne({
                  companyId: companyId,
                  name: elem.Company,
                  status: {
                    $in: status,
                ...
                companyController.push(datas, res);

                //find section Id

                let sectionIds = await Section.findOne({
                  departmentId: {
                    $in: departmentId,
                  },
                ...
                // Sub Section
                let subSectionIds = await SubSection.findOne({
                  sectionId: {
                    $in: sectionId,
                  },
                  name: '__',
                ...
                // Subsection updated appoinment...
                let updateSub = await SubSection.update(
                  {
              ...
              let userDe = await User.findOne({
                ...
                .select('staffId role subSkillSets')
                .lean();
                ...
                if (err) {
                  ...
                  fs.readFile(fileLocation, function (err, data) {
                    transporter.sendMail({
                      ...
                      body: 'mail content...',
                      attachments: [
                        { filename: 'attachment.csv', content: data },
                      ],
                    }),
  ...
  // User Integration daily
  async userIntegrationSecond(req, res) {
    try {
      let companyId = '5a9d162b36ab4f444b4271c8';
      // Get Company Data
      let companyData = await Company.findOne({
      ...
      if (day <= 9) day = '0' + day;
      let getYear = date.getFullYear();
      let timeStamp = `${getYear}${month}${day}${'13'}`;
      let daily;
            ...
            if (data[i].name.indexOf(`dailyMySATS${timeStamp}`) > -1) {
              ...
              sftp
                ...
                .then((fileData) => {
                  ...
                  __.log(writtenFileData, 'writtenFileData');
                });

              var newFile = await spawn('unzip', [
                '-P',
                'P@55word!',
                '-d',
                './public/',
  ...
  async userIntegrationweekly(req, res) {
    try {
      ...
      let getMiniutes = date.getMinutes();
      let getSeconds = date.getSeconds();
      let timeStamp = `${getYear}${getMonth}${getDate}${getHours}${getMiniutes}${getSeconds}`;
      ...
      sftp
        ...
        .then(() => {
          sftp
            .get('/ADWeeklyExtractUAT/weeklyMySATS20190412144033.zip')
            .then((fileData) => {
              __.log(fileData);
              let writtenFileData = fs.writeFileSync(
                'public/weeklyMySATS20190412144033.zip',
                fileData,
              );
              __.log(writtenFileData, 'writtenFileData');
            });
      ...
      var newFile = await spawn('unzip', [
        ...
        'P@55word!',
        '-d',
        './public/uploads/customForm',
        './public/weeklyMySATS20190412144033.zip',
      ]);
      let serverFile = `./public/uploads/customForm/weeklyMySATS20190412144033.csv`;
      var columns = [
        'SNo',
        'EmployeeNumber',
        'SamAccountName',
          ...
          if (excelData.length > 0) {
            ...
            for (let elem of userData) {
              if (elem.UserAccountControl == '546') {
                let userDe = await User.findOneAndUpdate(
                  {
                    companyId: companyId,
                    staffId: elem.EmployeeNumber.toLowerCase(),
                  },
  ...
  async sftpData() {
    ...
    sftp
      ...
      .then(() => {
        return sftp.list('/ADDailyExtractUAT');
      })
      .then(async (data) => {
        for (var i = 0; i < data.length; i++) {
          let daily;
          if (data[i].name.indexOf('dailyMySATS2019052104') > -1) {
            console.log('listing directory>>>>>>>>', data[i].name);
            daily = data[i].name;
            sftp.get(`/ADDailyExtractUAT/${data[i].name}`).then((fileData) => {
              __.log(fileData);
              let writtenFileData = fs.writeFileSync(
                `public/${daily}`,
                fileData,
...
schedule.scheduleJob('00 30 */20 * * * ', cron.userIntegrationFirst);
schedule.scheduleJob('00 30 */5 * * * ', cron.userIntegrationSecond);
