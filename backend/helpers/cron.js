      ...
      async function (err, array) {
        ...
        if (excelData.length) {
          ...
          for (const elem of excelData) {
            ...
            let status =
              elem.UserAccountControl == '512' ||
              elem.UserAccountControl == '66048'
                ? 1
                : elem.UserAccountControl == '546'
                ? 2
                : 0;
            let bu = !!elem.Department
            ...
            if (!!!user.parentBussinessUnitId) {
              ...
              await SubSection.update(
                ...
                { $addToSet: { appointments: user.appointmentId } },
              );
            }
            ...
            const addToSystemAdmin = async () => {
              await User.update(
                { role: { $in: systemAdminRolesIds }, companyId: companyId },
                {
                  $addToSet: {
            ...
            if (userDe) {
              ...
              updatedUserData = await User.findOneAndUpdate(
                { companyId: companyId, staffId: user.staffId.toLowerCase() },
                { $set: user },
...
class cron {
      ...
      if (challenges.length) {
        for (const challenge of challenges) {
          ...
          if (!!challenge.selectedChannel && 1 === challenge.criteriaType) {
            ...
            const wall = await Wall.findOne({ _id: challenge.selectedWall })
  ...
  async notification() {
    ...
    for (let eachNotification of notificationDetails) {
      let usersDeviceTokens = eachNotification.notifyOverAllUsers
        .map((x) => x.deviceToken)
        .filter(Boolean);
  ...
  async publishingPost() {
    ...
    for (let post of postList) {
      ...
      if (post.channelId != null && post.categoryId != null) {
        ...
        const decodedString = parser.fromString(post.content.title, {
          wordwrap: 130,
        });
    ...
    try {
      ...
      for (let companyData of companyList) {
        ...
        let previousUpdated = moment()
          .substract(companyData.pwdSettings.pwdDuration - 10, 'days')
          .utc()
          .format();
    ...
    await sftp
      ...
      .then(async (data) => {
        ...
        const filteredData = data.filter(
          (v) => -1 !== v.name.indexOf(`dailyMySATS${timeStamp}`),
        );
        for (const d of filteredData) {
          let daily = d.name;
          await sftp.get(`/ADDailyExtractPRD/${daily}`).then(
            async (fileData) => {
              let writtenFileData = await fs.writeFileSync(
                `public/${daily}`,
                fileData,
              );
              await spawn('unzip', [
  ...
  async integrateNow13(req, res) {
    ...
    await fs.readdir(currentFolder, async (err, files) => {
      ...
      const myFiles = files.filter(
        (v) => -1 !== v.indexOf(currentDate) && v.includes('.csv'),
      );
      if (myFiles.length) {
        ...
        await fs.appendFileSync(
          './public/integration/integration.log',
          JSON.stringify({ message: 'File Not found', date: new Date() }),
        );
      }
    });
  }
