...
const PushNotificationHelper = require('./pushNotificationHelper'); // done
const Agenda = require('./agenda'); // done
const PushNotification = require('./pushNotification'); // done
...
class ManageNotification {
    ...
    async getAll(where, requiredData, sortObj, skip, limit) {
        let notificationData = await Notification.find(where, requiredData).populate([
            {
                path: 'createdBy',
                select: 'name'
            }
    ...
    async getSingle(where) {
        let notificationData = await Notification.findOne(where).populate({
    ...
    async cancelled(id, notificationStatus) {
        const updateCron = await Notification.findOneAndUpdate({ _id: id }, { $set: { notificationStatus: notificationStatus } })

        return true;
    }
    async updateField(id, updateObj) {
        const updatedNotification = await Notification.findOneAndUpdate({ _id: id }, { $set: updateObj })
        return updatedNotification;
    }
    async updateOne(id, updateObj) {
        const updatedNotification = await Notification.findOneAndUpdate({ _id: id }, updateObj, { new: true })
        return updatedNotification;
    }
    async sendNotification(data) { // fromUpdate 
        if (data.otherModules) { // from doodleblue for other module push notifications
            const result = await this.sendPushNotificationOtherModules(data);
            return result;
        } else if (data.moduleType === 'pushNotification') {
            const result = await this.sendImmidateNotification(data._id, false)
            return result;
        } else {
            return "old data"
        }
    }
    async sendImmidateNotification(id, fromUpdate = false) { // fromUpdate 
        ...
        let notificationData = await Notification.findOne({ _id: id, isPublish: true, notificationStatus: { $nin: [3, 4] } }, getField);
        if (notificationData) {
            ...
            const users = await PushNotificationHelper.getAssignUsers(notificationData, ['_id','deviceToken']);
            let totalUser = 0; 
            let userId = [];
            if (users.length > 0) {
                ...
                const result = PushNotification.push(notificationObj, deviceTokenList, notificationData._id);
            }
            if (!fromUpdate) {
                const updateCron = await NotificationCron.updateOne({ 'data._id': id }, { $set: { lastFinishedAt: new Date(), totalSent: totalUser, 'data.notifyAcknowledgedUsers': userId } });
        ...
        const updateCron = await NotificationCron.updateOne(
            { 'data._id': notificationData.id },
            {
                $set: {
                    ...
                    'data.notifyAcknowledgedUsers': notificationData.notifyAcknowledgedUsers
                }
            });
    ...
    async createPushNotification(input) {
        ...
        await NotificationCron.deleteMany({ 'data.moduleId': input.moduleId, 'data.moduleAction': input.moduleAction , nextRunAt: { $ne: null } });

        // send immediatly, (nextRunTime: 'now')
        if (input.nextRunTime === 'now' && (input.pushNotificationStatus !== push_noti_status.FAILED)) {
        ...
        await Agenda.createJob('ad-hoc', input.nextRunTime, input, this.dbName);

        /* make sure next run time as null */

        return {
            status: input.pushNotificationStatus === push_noti_status.FAILED ? false : true,
            message: input.pushNotificationStatus === push_noti_status.FAILED ? input.reason : 'Push notification created successfully'
        }
    }
    ...
    async ifActiveThenSend(input) {
        ...
        if (!!output.deviceTokenList.length && (input.pushNotificationStatus !== push_noti_status.FAILED)) {
