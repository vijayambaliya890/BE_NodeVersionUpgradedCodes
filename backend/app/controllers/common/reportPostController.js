...
class ReportPost {
    async reportPost(req, res) {
        try {
            ...
            let requiredResult = await __.checkRequiredFields(req, ['postId'], 'reportPost');
            if (requiredResult.status == false)
                return __.out(res, 400, requiredResult.missingFields);

            let wallData = await WallPostModel
                .findOne({
                    _id: req.body.postId
                })
                .populate({
            ...
            if (!wallData)
                return __.out(res, 300, 'Invalid Wall record');
            ...
            if (!newReport)
                return __.out(res, 300, 'Error while reporting post');
            var query = {
                    _id: req.body.postId
                },
                update = {
            ...
            let isCountUpdated = await WallPostModel.findOneAndUpdate(query, update, options);
            if (!isCountUpdated)
    ...
    async reportCommment(req, res) {
        try {
            ...
            let requiredResult = await __.checkRequiredFields(req, ['commentId'], 'reportPost');
            if (requiredResult.status == false)
                return __.out(res, 400, requiredResult.missingFields);
            ...
            let isCountUpdated = await WallCommentModel.findOneAndUpdate(query, update, options);
            if (!isCountUpdated)
                return __.out(res, 300, 'Error while updating report count');
            ...
            let isAlreadyReported = await ReportCommentModel.find(insertComment).lean();
            if (isAlreadyReported.length > 0)
    ...
    async reportChannelComment(req, res) {
        try {
            ...
            let requiredResult = await __.checkRequiredFields(req, ['commentId', 'postId'], 'reportPost');
            if (requiredResult.status == false)
                return __.out(res, 400, requiredResult.missingFields);
            ...
            let reportIndex = commentData.reportList.findIndex(x => x.reportedBy.toString() == req.user._id);
            if (reportIndex > -1) {
    ...
    async reportChannelPost(req, res) {
        try {
            ...
            if (!postData)
                return __.out(res, 300, 'Invalid News/Event record');
