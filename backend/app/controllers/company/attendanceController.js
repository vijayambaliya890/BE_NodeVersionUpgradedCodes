...
class attendanceController {
    ...
    async updateRedisSingle(res, businessUnitId, shiftDetailId) {
        try {
            ...
            const ree = await Promise.all([redisData.readModifyAshishSingleShift(businessUnitId, shiftDetailId), redisData.historySingle(businessUnitId, shiftDetailId)],
                redisData.history(businessUnitId))
    ...
    async getLogs(req, res) {
        try {
            let data = await AttendanceLog.find({ businessUnitId: req.body.businessUnitId }, { createdAt: 1, shiftDetailId: 1, attendanceMode: 1, status: 1, userId: 1, businessUnitId: 1 }).populate([{
                path: 'userId',
                select: 'staffId name'
            }, {
                path: 'businessUnitId',
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                item.buName = item.businessUnitId.sectionId.departmentId.companyId.name + ' > ' +
                ...
                item.shiftStartTime = this.getDateFormat(item.shiftDetailId.startTime, timeZone);// item.shiftDetailId.startTime;
                item.shiftEndTime = this.getDateFormat(item.shiftDetailId.endTime, timeZone);// item.shiftDetailId.endTime;
            ...
            if (keys.length > 0) {
                json2csv({ data: dataFormat, fields: keys }, function (err, csv) {
                    ...
                    res.setHeader('Content-disposition', 'attachment; filename=attendancelog.csv');
                            ...
                            Attendance.find({
                                ...
                                .then(async (attendanceResult) => {
                                    ...
                                    if (attendanceResult.length === 0) {
                                        ...
                                        new AttendanceLog(req.body).save().then((log) => {
                                        ...
                                        new Attendance(req.body).save().then(async (result) => {
                                            ...
                                            const rR = await this.updateRedisSingle(res, shiftData.businessUnitId, req.body.shiftDetailId)
                                            console.log("rR", rR)
    ...
    async autoApprove(req, res) {
        try {
            ...
            // status:2
            let attendanceData = await Attendance.find({ _id: req.body.id }).populate([
                {
                    path: 'shiftDetailId',
            ...
            for (let ii = 0; ii < len; ii++) {
                ...
                if (buData.length === 0) {
                    console.log('aaaa')
                    buInfo = await SubSection.findOne({ _id: buId }, { breakInMinutes: 1, shiftTimeInMinutes: 1, isBreakTime: 1, shiftBreak: 1 });
                ...
                buInfo.shiftBreak.sort(function (a, b) { return a.shiftHour - b.shiftHour });
                ...
                if (data.shiftDetailId.isExtendedShift) {
                    const isStaffPresent = data.shiftDetailId.extendedStaff.filter((isStaff) => {
                        return isStaff.userId == data.userId;
                    });
                    if (isStaffPresent.length > 0) {
                        console.log('present hai')
                        const normalHr = (new Date(data.shiftDetailId.endTime) - new Date(data.shiftDetailId.startTime)) / (1000 * 60 * 60);
                        const normalHrOT = (new Date(isStaffPresent[0].endDateTime) - new Date(isStaffPresent[0].startDateTime)) / (1000 * 60 * 60);
                ...
                var diff = new Date(data.clockInDateTime).getTime() - new Date(data.shiftDetailId.startTime).getTime();
                const diffMin = diff / 60000;
                let clockinDiff = diffMin;
                if (diffMin < 0) {
                    clockinDiff = (-1) * diffMin;
                }
                let clockoutDiff = new Date(data.clockOutDateTime).getTime() - new Date(data.shiftDetailId.endTime).getTime();
                clockoutDiff = clockoutDiff / 60000;
                let clockShiftDuration = new Date(data.clockOutDateTime).getTime() - new Date(data.clockInDateTime).getTime();
                clockShiftDuration = clockShiftDuration / 60000;
                let shiftDuration = new Date(data.shiftDetailId.endTime).getTime() - new Date(data.shiftDetailId.startTime).getTime();
                ...
                if (clockinDiff <= buInfo.shiftTimeInMinutes && clockoutDiff <= buInfo.shiftTimeInMinutes && (!buInfo.isBreakTime || data.totalBreakDuration >= buInfo.breakInMinutes)) {
                    ...
                    const obj = {
                        ...
                        approval: {
                            ...
                            "isAutoApprove": true,
                            "shift": false,
                            "approveClockInTime": new Date(data.shiftDetailId.startTime).toISOString(),
                            "approveClockOutTime": new Date(data.shiftDetailId.endTime).toISOString(),
        ...
        try {
            ...
            Attendance.findById(req.body.attendanceId).then((attendanceInfo) => {
                ...
                Attendance.findOneAndUpdate({ _id: mongoose.Types.ObjectId(req.body.attendanceId) }, {
                    $set: {
                        clockOutDateTime: obj.clockOutDateTime, status: obj.status, duration
                    }
                    ...
                    if (result) {
                        ...
                        const re = await this.updateRedisSingle(res, result.businessUnitId, result.shiftDetailId);
                        console.log("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ after redis", re)
                        this.updateRedis(result.businessUnitId, 'add');
                        new AttendanceLog(logs).save().then((log) => {
    ...
    async updateBreakTime(req, res) {
        try {
                    ...
                    if (attendanceData) {
                        ...
                        if (attendanceData.breakTime && attendanceData.breakTime.length > 0) {
                            attendanceData.breakTime.forEach((bt) => {
                                if ((new Date(startTimeDate).getTime() >= new Date(bt.startTime).getTime() && new Date(startTimeDate).getTime() < new Date(bt.endTime).getTime())
                                    || (new Date(endTimeDate).getTime() >= new Date(bt.startTime).getTime() && new Date(endTimeDate).getTime() <= new Date(bt.endTime).getTime())) {
                        ...
                        if (!isBreakTimeOverLap) {
                            Attendance.findOneAndUpdate({
                                ...
                                //console.log('result', result);
                                if (result) {
                                    await updateRedisSingle(res, result.businessUnitId, result.shiftDetailId);
                            ...
                            return res.json({ status: 10, data: null, message: "Break Time is overlapping" });
                        }
                    } else {
                        return res.json({ status: 2, data: null, message: "No Clock in attendance found" });
                    }
            ...
            try{ 
                // redisData.history(businessUnitId)
                console.log("before redis setting data")
                const ree = await Promise.all([redisData.readModifyAshishSingleShift(businessUnitId, shiftDetailId), redisData.timesheetDataSingleShift(businessUnitId, shiftDetailId)])
                // const r = await redisData.readModifyAshishSingleShift(businessUnitId, shiftDetailId);
                // const p = await redisData.timesheetDataSingleShift(businessUnitId, shiftDetailId);
                console.log("after redis setting data", ree)
                return ree;
    ...
    async check(req, res) {
        try {
            Attendance.find({
                userId: mongoose.Types.ObjectId(req.params.userId), shiftDetailId:
                    mongoose.Types.ObjectId(req.params.shiftDetailId)
            }, { status: 1, attendanceMode: 1 }).then((result) => {
                ...
                Attendance.findOne({
                    ...
                    if (results.shiftDetails.extendedStaff.length > 0) {
                        results.shiftDetails.extendedStaff = results.shiftDetails.extendedStaff.filter((item) => {
                            return item.userId.toString() === req.params.userId && (item.confirmStatus == 1 || item.confirmStatus == 2)
                        });
                    }
                    var otDuration = 0;
                    var normalDuration = 0;
                    results.attendance = attendance;
                    if (results && results.attendance && (results.attendance.approval.neither || results.attendance.approval.shift
                    ...
                    if (req.params.splitShiftId === '0') {
                        return res.json({ status: 1, message: 'Data Found', data: results, otDuration, normalDuration });
                    } else {
        ...
        try {
            ...
            function markBreakTime(shiftDetailId) {
                ...
                Attendance.findOneAndUpdate({
                    ...
                    console.log('result', result);
                    if (result) {
                        await this.updateRedisSingle(res, result.businessUnitId, result.shiftDetailId);
                        this.updateRedis(result.businessUnitId, 'add');
                        return res.json({ status: 1, data: result, message: "Break Time Updated successfully" });
                    } else
...
async function autoApproveCron() {
    try {
        ...
        tommrow = new Date(tommrow.setDate(tommrow.getDate() + 1));
        // status:2
        let attendanceData = await Attendance.find({ status: 2, createdAt: { $gte: yestrday, $lte: tommrow } }).populate([
            {
                path: 'shiftDetailId',
                select: 'startTime endTime duration isExtendedShift extendedStaff'
            }
