...
class subCategory {
    ...
    async create(req, res) {
        try {
            ...
            let requiredResult = await __.checkRequiredFields(req, ['name', 'status', 'categoryId']);
            if (requiredResult.status === false) {
                ...
                categoryController.push(params, res); /* push generated city id in state table (field name : subCategoryIds)*/
                this.read(req, res); /*calling read fn with subCategoryId(last insert id). it calls findOne fn in read */
            }
        } catch (err) {
            __.log(err);
            __.out(res, 500);
    ...
    async read(req, res) {
        try {
            ...
            let subCategories = await findOrFindOne.populate({
                path: 'categoryId',
                select: '_id name'
            }).lean();
            __.out(res, 201, {
            ...
            if (requiredResult.status === false) {
                ...
                if (doc === null) {
                    ...
                    if (req.body.categoryId && doc.categoryId != req.body.categoryId) {
                        ...
                        categoryController.pull(params, res); /* pull this city id in from existing state (field name : subCategoryIds)*/
                    }
