...
class appointment {
    async create(req, res) {
        try {
            ...
            if (requiredResult.status === false) {
                ...
                this.read(req, res); /*calling read fn with appointmentId(last insert id). it calls findOne fn in read */
            }
        } catch (err) {
            __.log(err);
            __.out(res, 500);
    ...
    async readWithPn(req, res) {
        try {
            ...
            const appointments = await Appointment.find(query).skip(page).limit(limit).lean();
            appointments.forEach((a, i) => a.sno = page + i + 1)
            return res.status(201).json({ appointments, recordsTotal, recordsFiltered });
        } catch (error) {
            __.log(error);
    ...
    async getAppointments(req, res){
        try {
            ...
            let appointments = await Appointment.aggregate([query,{
                $project:{name:1}
            }, {
                $sort:{
                    name:1
