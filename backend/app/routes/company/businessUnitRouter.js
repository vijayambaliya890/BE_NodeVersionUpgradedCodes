...
//RENDER
businessUnitRouter.get('/getBusinessUnits', businessUnitController.getBusinessUnits);
businessUnitRouter.post('/getPlannedHrs', businessUnitController.getBusinessUnitDetails);

businessUnitRouter.use(passport.authenticate('jwt', {
    session: false
}), /*Allow only admin*/
    (req, res, next) => {
        if (req.user.isFlexiStaff !== 1 || req.path.includes('weekNo'))
            next();
        else
...
businessUnitRouter.post('/updateskillsetandlocation', __.checkRole('businessUserSetup').validate, (req, res) => {
    businessUnitController.updateSkillSetAndLocation(req, res)
});

businessUnitRouter.post('/updateBuShiftScheme', __.checkRole('businessUserSetup').validate, (req, res) => {
