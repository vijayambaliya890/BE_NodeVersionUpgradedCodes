...
mongoose.connection.on("error", (err) => {
  console.log("%s MongoDB connection error. Please make sure MongoDB is running (" + db_host + ").", chalk.red("✗"));
  process.exit();
});
console.log("DATABASE CONNECTED IS: ", db_host);
...
app.use(
  ...
  function (req, res, next) {
    if (!!req.user && req.user.pwdDurationStatus && req.user.pwdDurationStatus == true) {
      next();
    } else {
      res.send(401, {
...
app.listen(app.get("port"), (req, res) => {
  console.log(`%s App is running at ${__.serverBaseUrl()} `, chalk.blue("✓"), app.get("port"), environment);
