...
class channel {
  async create(req, res) {
    try {
      ...
      if (newField) return __.out(res, 200, 'Field Created');
    } catch (err) {
      __.log(err);
  ...
  async remove(req, res) {
    try {
      ...
      var removedField = await UserField.findOneAndUpdate(
        where,
        {
          $set: {
            status: 3,
  ...
  async read(req, res) {
    try {
      ...
      var fieldList = await UserField.find(where)
        .sort({
          indexNum: 1,
        })
        .lean();
  ...
  async getAll(req, res) {
    try {
      ...
      var data = await this.getAllCustomFields(where, req.query);
  ...
  async getAllCustomFields(
    where,
    { page, limit, search, sortBy, sortWith, statusFilter },
  ) {
    let searchCondition = {};
    if (search) {
      searchCondition['fieldName'] = { $regex: search, $options: 'i' };
    }
    statusFilter = (statusFilter && [statusFilter]) || ['active', 'inactive'];
